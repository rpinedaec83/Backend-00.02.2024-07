class Reparacion {
    constructor(usuario, problema, especialista, autorizacion, pago) {
        ...
        this.pago = pago;
        this.estadoActual = 0; // Variable de control del estado
        this.estado = 'INICIO';
    }
    ...
    avanzarEstado() {
        const estados = ['INICIO', 'EN PROCESO', 'POR ENTREGAR', 'ENTREGADO'];
        if (this.estadoActual < estados.length - 1) {
            this.estadoActual++;
            this.estado = estados[this.estadoActual];
        }
    }
}
...
document.getElementById('login-form').addEventListener('submit', function(event) {
    ...
    // Cargar la reparación desde el almacenamiento
    reparacion = cargarDesdeStorage();
    if (!reparacion) {
        alert('No hay información de reparación guardada. Por favor, registre primero.');
        return;
    }
    ...
    alert(`Estado del equipo: ${reparacion.estado}`);
    console.log(`Estado actual del equipo: ${reparacion.estado}`);

    // Mostrar la información adicional en la interfaz
    document.getElementById('estado').innerText = reparacion.estado;
    document.getElementById('especialista-estado').innerText = reparacion.especialista.nombre;
    document.getElementById('ubicacion-estado').innerText = reparacion.especialista.sucursal;
...
window.addEventListener('load', function() {
    reparacion = cargarDesdeStorage();
    if (reparacion) {
        console.log('Reparación encontrada en storage:', reparacion);
    }
});
...
function cargarDesdeStorage() {
    const data = localStorage.getItem('reparacion');
    if (data) {
        const obj = JSON.parse(data);
        // Reconstruir el objeto como una instancia de la clase Reparacion
        const reparacion = new Reparacion(
            new Usuario(obj.usuario.nombre, obj.usuario.correo, obj.usuario.telefono, obj.usuario.imei, obj.usuario.sucursal),
            obj.problema,
            new Especialista(obj.especialista.nombre, obj.especialista.sucursal, obj.especialista.skills),
            new Autorizacion(obj.autorizacion.autorizacionEscrita, obj.autorizacion.montoAbonar),
            new Pago(obj.pago.costoTotal, obj.pago.abono)
        );
        reparacion.estadoActual = obj.estadoActual; // Restaurar el estadoActual
        reparacion.estado = obj.estado; // Restaurar el estado
        return reparacion;
    }
    return null;
}
