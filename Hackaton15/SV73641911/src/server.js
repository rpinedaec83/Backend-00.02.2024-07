...
function chat_start() {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('send-message', function (data, user) {
            // Encontrar el id del usuario y guardar el mensaje
            var loginId = "SELECT id FROM login WHERE username = '" + user + "'";
            con.query(loginId, function (err, result) {
                if (err) throw err;
                var login_id = result[0]["id"];
                // console.log(login_id);
                var sql = "INSERT INTO message (message , user, login_id) VALUES ('" + data + "' , '" + user + "', '" + login_id + "')";
                con.query(sql, function (err, result) {
                    if (err) throw err;
                //console.log("1 record inserted");
                });
